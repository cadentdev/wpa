...
Since v0.8.1, WPA detects this pattern (an HTML error page from a REST endpoint) and reports it as a **possible WAF block** instead of dumping raw HTML. See [docs/waf-compatibility.md](docs/waf-compatibility.md) for the full symptom matrix.
...
This only affects `DELETE` requests. `GET` (list) and `POST` (create, update) work with the default Wordfence configuration — except list queries using the `?author=` parameter (`wpa post list --author <id>`), which Wordfence's author-enumeration protection rejects with an HTML 404.
...
If `wpa user create --password-stdin` returns a `400` error with a password strength message, use a stronger password. Passwords generated by WPA (the default when `--password-stdin` is omitted) always satisfy these requirements.

### Creating users: passwords and notification emails

As of v0.8.1, `wpa user create` no longer accepts `--password` on the command line (it leaked via `ps(1)`, shell history, and CI logs) and no longer prompts interactively. The two paths are:

- **No password flag (recommended):** WPA generates a strong random password and never displays it. Pair with `--send-email` so the new user receives a one-time set-password link and chooses their own password.
- **`--password-stdin`:** reads the password from stdin, e.g. `printf '%s\n' "$PW" | wpa user create ... --password-stdin`.

**Important:** the WordPress REST API cannot send the "new user" notification email — that email comes from `wp-admin`'s own code, outside the REST path. Without `--send-email`, the account exists and **nobody is told**. The `--send-email` flag triggers WordPress core's lost-password flow (`wp-login.php?action=lostpassword`), which emails a genuine one-time set-password link. Two caveats:

- The subject line reads *Password Reset*, not *Login Details*.
- Security plugins may rate-limit or block `wp-login.php` POSTs. WPA verifies WordPress accepted the request and warns loudly if it didn't — but a send is not a delivery, so confirm with the recipient or the site's mail log for anything important.

### TLS troubleshooting

WPA refuses any response whose final URL downgraded from `https://` to `http://`, because credentials would travel unencrypted. If you see the *"connection was downgraded from https to http"* error:

1. **Check the site URL** in `~/.config/wpa/<site>/.env` — it must be the site's canonical address. A common cause is `https://example.com` when the site canonicalizes to `https://www.example.com` (or vice versa) via an intermediate `http://` hop.
2. **Check server redirects** — a misconfigured load balancer, CDN, or `.htaccess` rule may redirect API requests to `http://`. Test with `curl -sI https://example.com/wp-json/ | grep -i location`.
3. **If neither explains it**, treat the error seriously: an unexpected scheme downgrade is the signature of a man-in-the-middle. Verify from a different network before retrying.
